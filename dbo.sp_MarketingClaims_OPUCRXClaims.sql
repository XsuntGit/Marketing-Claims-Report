...
ALTER PROCEDURE [dbo].[sp_MarketingClaims_OPUCRXClaims]
AS
SET NOCOUNT ON;
BEGIN TRY

	EXEC [dbo].[sp_sys_MarketingClaims_ProcessLog] 'sp_MarketingClaims_OPUCRXClaims start.'
	...
	TRUNCATE TABLE [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage]

	--ALTER INDEX [idx_MarketingClaims_MarketingOPUCRXClaims_Stage] ON [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage] DISABLE

	-- OPU/MCK claims...
	INSERT INTO [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage]
	SELECT --DISTINCT
		CreatedAt,
		Status,
		...
		null as ClaimNumber,
		a.tblclaimID as ClaimTransactionNumber,								--Hao, 2/21/2018
		case Status
	...
	FROM MarketingClaims_MarketingClaimsRaw a with (nolock)
	...
	LEFT JOIN [Enbrel_Production].[dbo].[tblProgram] f with (nolock)			--2/23/2018, used to be join
		ON a.PatientProgramType = f.ProgramID
	LEFT JOIN [Enbrel_Production].[dbo].[tblNDCList] ndc with (nolock)
		ON a.NDC = ndc.NDCCode
	WHERE a.PatientID not in ('50491','46631','50492','51757','51758')
		AND a.cardid is not null

	
	UPDATE a
	SET a.ClaimNumber = b.ClaimIDX
	FROM [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage] a
	...
	--ALTER INDEX [idx_MarketingClaims_MarketingOPUCRXClaims_Stage] ON [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = COLUMNSTORE)
	...
	FROM [dbo].[MarketingClaims_MarketingOPUCRXClaims_Stage] (nolock) as tbl
	CROSS APPLY dbo.BreakStringIntoRows(RejectionCode)

	EXEC [dbo].[sp_sys_MarketingClaims_ProcessLog] 'sp_MarketingClaims_OPUCRXClaims end.'
