...
ALTER PROCEDURE [dbo].[sp_MarketingClaims_MapMasterClaimID]
AS
SET NOCOUNT ON;
BEGIN TRY

	EXEC [dbo].[sp_sys_MarketingClaims_ProcessLog] 'sp_MarketingClaims_MapMasterClaimID start.'
	...
	FROM
	(
		SELECT distinct TeradataVendorCode, CardID, TeradataClaimReferenceNumber
		FROM [dbo].[MarketingClaims_MarketingClaimsRaw]
	) a
	...
	EXEC [dbo].[sp_sys_MarketingClaims_ProcessLog] 'sp_MarketingClaims_MapMasterClaimID end.'
